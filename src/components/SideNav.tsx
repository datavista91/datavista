...
const SideNav = ({ isOpen, setIsOpen }: SideNavProps) => {
  const navItems = [
    ...
    { name: 'Share', icon: <Share2 size={20} />, path: '/share' },
    { name: 'Settings', icon: <Settings size={20} />, path: '/settings' },
  ];

  return (
    <div className={`${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:relative inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out flex flex-col`}>
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <Link to="/" className="flex items-center space-x-2">
          ...
          <div>
            <div className="font-bold text-lg" style={{ fontFamily: 'Poppins, sans-serif' }}>AIDash</div>
